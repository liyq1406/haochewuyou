...
public class TodayNewArrivalFragment extends HCBaseFragment {
  ...
  @InjectView(R.id.hcptr_today_list) HCPullToRefresh mPullToRefresh;

  @InjectView(R.id.linear_buycar_empty_parent_new_arrival) LinearLayout mEmptyView;
  ...
  @InjectView(R.id.linear_today_parent) LinearLayout mFilterParentLinear;
  ...
  private Handler timeHandler = new Handler() {
    @Override public void handleMessage(Message msg) {
      mTodayAdapter.timeCurrent = System.currentTimeMillis() / 1000;
  ...
  @Override void doInitViewOrData() {
    ...
    if (!isTodayListVisible2User()) return;
    initCommonFilter();
    initPullToRefresh();
    initRefreshListener();
    initScrollListener();
  ...
  private boolean isTodayListVisible2User() {
    ...
    return coreFragmentXT.isVisible() && (page == CoreFragment.PAGE_TODAY_VEHICLES);
  }
  ...
  private void initCommonFilter() {
    ...
    trans.add(R.id.frame_today_parent, mFilterFragment, host);
  ...
  private void initPullToRefresh() {
    ...
    mTodayAdapter = new NewArrivalAdapter(getActivity(), mTodayDatas, itemRes);
    mPullToRefresh.setAdapter(mTodayAdapter);
  ...
  private void initRefreshListener() {
    mPullToRefresh.setOnRefreshCallback(new HCPullToRefresh.OnRefreshCallback() {
      @Override public void onPullDownRefresh() {
        mCurrentPage = 0;
        requestTodayVehicles(mCurrentPage);
      }
  ...
  private void computeTotal(int count) {
    ...
    if (mTotalPage == 0 && mTodayDatas.size() > 0) {
      mTotalPage = 1;
    }
  }
        ...
        if (scrollState == HCPullToRefresh.SCROLL_STATE_TOUCH_SCROLL
          ...
          if (mTotalPage > 0) {
            ...
            try {
              ...
              HCLog.d(TAG, "TodayNewArrival onScrollStateChanged is crash");
            }
          }
        }
      }
  ...
  private void onFilterChanged() {
    if (mPullToRefresh == null) return;
    if (!FilterUtils.getTodayFilterTerm().equals(mLastTerm)) {
      mPullToRefresh.autoRefresh();
    }
  }

  private void requestTodayVehicles(final int page) {
    Map<String, Object> params = HCParamsUtil.getTodayVehicleSouceList(page);
    API.post(new HCRequest(params, new HCSimpleCallBack() {
      @Override public void onHttpFinish(String responseJsonString) {
        handleTodayVehicles(responseJsonString);
      }
    }));

    mLastTerm = FilterUtils.getTodayFilterTerm();
  }

  private synchronized void handleTodayVehicles(String responseJsonString) {
    ...
    if (!TextUtils.isEmpty(responseJsonString)) {
      try {
        ...
        if (mCurrentPage == 0) {
          ...
          if (!isSortChangedData && count > 0 && isTodayListVisible2User()) {
            HCUtils.showCountToast(count);
          }
        }
        if (!HCUtils.isListEmpty(datas)) {
          datas = removeDuplicateData(mTodayDatas, datas);
          mTodayDatas.addAll(datas);
        ...
        HCLog.d(TAG, "handleTodayVehicles is crash ...");
      }
    }
   ...
   * 试试帮买服务
   */
  @OnClick(R.id.btn_buycar_empty_helpbuy_new_arrival) @SuppressWarnings("unused")
  ...
  @Override public void onEvent(HCCommunicateEntity entity) {
    ...
    switch (action) {
      ...
      case HCEvent.ACTION_SWAPTO_CORE_INNER:
        if (CoreFragment.PAGE_TODAY_VEHICLES == entity.getIntValue()) {
          if (!isInitCompleted) {
            doInitViewOrData();
          }
        }
      ...
      case HCEvent.ACTION_DUPLICATE_CLICK_TAB: {
        ...
        if (FragmentController.CORE_VEHICLE_TAG.equals(tab)) {
          if (getParentFragment() instanceof CoreFragment) {
            CoreFragment coreFragmentXT = (CoreFragment) getParentFragment();
            int page = coreFragmentXT.getCurrentVisiblePage();
            if (CoreFragment.PAGE_TODAY_VEHICLES == page) {
