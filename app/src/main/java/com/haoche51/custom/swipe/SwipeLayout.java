package com.haoche51.custom.swipe;
...
public class SwipeLayout extends FrameLayout {
  ...
  public SwipeLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.SwipeLayout);
    int dragEdgeChoices =
    ...
    setClickToClose(
        a.getBoolean(R.styleable.SwipeLayout_clickToClose, mClickToClose));
    ...
    int ordinal =
        a.getInt(R.styleable.SwipeLayout_show_mode, ShowMode.PullOut.ordinal());
  ...
  protected boolean isViewTotallyFirstShowed(View child, Rect relativePosition, DragEdge edge,
    ...
    if (getShowMode() == ShowMode.LayDown) {
      if ((edge == DragEdge.Right && surfaceRight <= childLeft) || (edge == DragEdge.Left
          && surfaceLeft >= childRight) || (edge == DragEdge.Top && surfaceTop >= childBottom) || (edge == DragEdge.Bottom && surfaceBottom <= childTop)) {
        r = true;
      }
    } else if (getShowMode() == ShowMode.PullOut) {
      if ((edge == DragEdge.Right && childRight <= getWidth()) || (edge == DragEdge.Left
          && childLeft >= getPaddingLeft()) || (edge == DragEdge.Top && childTop >= getPaddingTop())
  ...
  /***
   * Allows to stablish at what percentage of revealing the view below should the view finish
   * opening
   * if it was already open before dragging
   *
   * @param willOpenPercentAfterOpen The percentage of view revealed to trigger, default value is
   * 0.25
   */
   ...
   * if it was already closed before dragging
   *
   * @param willOpenPercentAfterClose The percentage of view revealed to trigger, default value is
   * 0.75
   */
  ...
  private boolean performAdapterViewItemLongClick() {
    ...
    if (t instanceof AdapterView) {
      ...
      try {
        Method m = AbsListView.class.getDeclaredMethod("performLongPress", View.class, int.class,
            long.class);
        ...
        if (view.getOnItemLongClickListener() != null) {
          handled =
              view.getOnItemLongClickListener().onItemLongClick(view, SwipeLayout.this, p, vId);
        }
        if (handled) {
   ...
   */
  @Nullable public View getCurrentBottomView() {
    List<View> bottoms = getBottomViews();
  ...
  public Status getOpenStatus() {
    ...
    if (surfaceLeft == (getPaddingLeft() - mDragDistance) || surfaceLeft == (getPaddingLeft() + mDragDistance) || surfaceTop == (getPaddingTop() - mDragDistance) || surfaceTop == (
